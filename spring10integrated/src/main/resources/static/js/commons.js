...
/*
    ...
    - .page 내부에 .btn-prev를 배치하여 이동버튼을 구현
    - 첫번째 이전버튼과 마지막 다음버튼은 자동 제거되므로 반드시 만들 것
*/

$(function(){
    //[1] 모든 페이지를 다 숨긴고 1페이지 표시
    $(".page").hide().first().show(); //권장
    // $(".page").hide().eq(0).show();
    // $(".page:gt(0)").hide(); //권장하지 않음

    //(추가) 첫번째 이전버튼과 마지막 다음버튼을 삭제
    $(".page").find(".btn-prev").first().remove(); //페이지에 있는 버튼만 적용시키겠다
    $(".page").find(".btn-next").last().remove(); //페이지에 있는 버튼만 적용시키겠다

    //[2] 다음 버튼을 누르면 버튼이 있는 페이지는 숨기고 다음 페이지를 표시
    $(".btn-next").click(function(){
        // $(this).parent(".page").hide().next(".page").show(); //페이지 바로 밑에 있어야함
